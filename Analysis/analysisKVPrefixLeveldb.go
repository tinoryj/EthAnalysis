...
func main() {
	...
	if _, corrupted := err.(*errors.ErrCorrupted); corrupted {
		log.Printf("Target database may be corrupted, start recovery\n")
		db, err = leveldb.RecoverFile(file, nil)
		log.Printf("Target database may be corrupted, recovery done\n")
	}
	if err != nil {
		log.Fatalf("Can not open target database, err: %v\n", err)
	}
	defer db.Close()
