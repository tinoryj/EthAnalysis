...
func processLogFile(filePath string, progressInterval uint64, startBlockNumber, endBlockNumber, stepSize uint64) {
	...
	for {
		...
		outPutLogPath := "countKVDis-" + strconv.FormatUint(currentStartBlockNumber, 10) + "_" + strconv.FormatUint(currentEndBlockNumber, 10) + ".txt"
...
func main() {
	if len(os.Args) < 6 {
		fmt.Println("Usage: program <log_file_path> <step_size> <progress_interval> <start_block_number> <end_block_number>")
		return
	}
