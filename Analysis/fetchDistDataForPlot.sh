...
TargetDataTypes=("SnapshotNodeAccountPrefix" "SnapshotNodeStoragePrefix" "TrieNodeAccountPrefix" "TrieNodeStoragePrefix" "TxLookupPrefix")
...
for TargetDataType in "${TargetDataTypes[@]}"; do
    for TargetOpType in "${TargetOpTypes[@]}"; do
        for TargetDrawPonitNumber in "${TargetDrawPonitNumbers[@]}"; do
            echo "Processing ${TargetDataType}..."
            filePathWithoutCache="${PATH_TO_WITHOUT_CACHE}/${TargetDataType}_${TargetOpType}_without_key_dis.txt"
            filePathWithCache="${PATH_TO_WITH_CACHE}/${TargetDataType}_${TargetOpType}_with_key_dis.txt"
