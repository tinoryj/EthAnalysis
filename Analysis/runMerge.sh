...
for category in "${categorySet[@]}"; do
    ...
    canContinue=false
        ...
        for filePathPrefix in "${filePathPrefixSet[@]}"; do
            if [ ! -f "${filePathPrefix}/${category}_${opType}_dis.txt" ]; then
                echo "File ${filePathPrefix}/${category}_${opType}_dis.txt does not exist"
                continue
            fi
            echo "${filePathPrefix}/${category}_${opType}_dis.txt" >> $currentLogFileName
            canContinue=true
        done
